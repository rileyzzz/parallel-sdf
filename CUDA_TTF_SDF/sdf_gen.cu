...
__global__ void sdf_calc(int width, int height, unsigned char* data, unsigned char* out)
{
	int x = threadIdx.x;
	int y = blockIdx.x;
...
#define WRITE(_x, _y, val) out[_y * stride + _x] = val
	...
	WRITE(x, y, (unsigned char)(dist * 255.0f));
...
void CUDA_SDF::GenerateSDF(SDFGenerationContext& ctx)
{
	...
	sdf_calc << <numBlocks, blockSize >> > (ctx.width, ctx.height, ctx.data, ctx.out_data);
